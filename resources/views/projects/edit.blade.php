        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <!-- Edit Form -->
                    <form class="mt-6 space-y-6" action="{{route('projects.edit',$project->id)}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div>
                        ...
                        <div>
                            ...
                            <x-text-input id="first_device_id" name="first_device_id" type="number" class="mt-1 block w-full"
                                          value="{{ old('first_device_id') ?? $project->first_device_id}}"
                                          required autofocus/>
                        ...
                        <div>
                            ...
                            <x-text-input id="csv_file" name="csv_file" type="file" class="mt-1 block w-full" value="{{old('csv_file') ?? $project->csv_file}}"
                                          required/>
                        ...
                        <div>
                            ...
                            <x-text-input id="image" name="image" type="file" class="mt-1 block w-full" value="{{ old('image') ?? $project->background_image }}"
                                          required/>
                            <x-input-error class="mt-2" :messages="$errors->get('image')"/>
                        </div>

                        <input type="hidden" value="{{old('x1') ?? $project->x1}}" name="x1">
                        <input type="hidden" value="{{old('y1') ?? $project->y1}}" name="y1">
                        <input type="hidden" value="{{old('x2') ?? $project->x2}}" name="x2">
                        <input type="hidden" value="{{old('y2') ?? $project->y2}}" name="y2">
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100" id="preview_holder">
                    <h1 class="text-2xl font-bold">Preview</h1>
                    <div class="mt-6" style="position: relative">
                        <img id="image_preview" src="{{ asset('storage/images/'. $project->background_image) }}">
...
<script>
    $(document).ready(function () {
        ...
        ctx.lineWidth = 10;
        var startX = $('input[name="x1"]').val();
        var startY = $('input[name="y1"]').val();
        var endX = $('input[name="x2"]').val();
        var endY = $('input[name="y2"]').val();
        ctx.beginPath();
